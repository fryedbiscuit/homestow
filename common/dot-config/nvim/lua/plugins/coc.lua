return {
	...
	config = function()
		...
		vim.g.coc_user_config = {
			-- 1. Truncate the function name itself if it's too long
			["suggest.labelMaxWidth"] = 30,

			-- 2. Truncate the "detail" (e.g., return types, [std::thread])
			["suggest.detailMaxLength"] = 30,

			-- 3. Strictly limit the "explanation" (documentation) window width
			["suggest.maxPreviewWidth"] = 50,

			-- 4. A hard global limit for ALL floating windows
			["coc.preferences.maxFloatWidth"] = 60,

			-- Ensure the float window is positioned to the right if possible
			["suggest.floatConfig"] = {
			["maxWidth"] = 50,
			["fixed"] = true
		}
